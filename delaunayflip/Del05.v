...
Require Export Del04.

(*===================================================
            CONNECTIVITY IN HMAPS : eqc 
===================================================*)
...
(* EQUIVALENCE WITH expf: *)

Lemma expf_expof:forall (m:fmap)(z t:dart),
  inv_hmap m -> 
     (expf m z t <-> expof m z t).
Proof.
intros.
unfold expf in |- *.
unfold expof in |- *.
...
Fixpoint nf(m:fmap):Z :=
 match m with
  ...
  | I m0 x _ _ => nf m0 + 1
  | L m0 zero x y =>
      let x_1:= cA_1 m0 one x in nf m0 +
       if expf_dec m0 x_1 y then 1 else -1
  | L m0 one x y =>
      let y0 := cA m0 zero y in nf m0 +
...
(* The Euler-Poincare characteristic is even: *)
...
(* IN FACT, WE EARLY HAVE THE SYNTHESIS: *)

Theorem genus_variation_0: forall(m:fmap)(x y:dart),
  let m1:= L m zero x y in
   inv_hmap m1 ->
    genus m1 = genus m +
       if eqc_dec m x y
       then 
           if expf_dec m (cA_1 m one x) y
           then 0
           else 1
       else 0.
Proof.
...
elim (expf_dec m (cA_1 m one x) y).
 intros.
   elim b.
   clear b.
   apply eqc_trans with (cA_1 m one x).
  apply (eqc_cA_1_r m one x H0 H2).
 apply expf_eqc.
   tauto.
 unfold expf in a.
    tauto.
...
Theorem genus_variation_1: forall(m:fmap)(x y:dart),
  let m1:= L m one x y in
   inv_hmap m1 ->
    genus m1 = genus m +
       if eqc_dec m x y
       then 
           if expf_dec m x (cA m zero y)
           then 0
           else 1
       else 0.
Proof.
